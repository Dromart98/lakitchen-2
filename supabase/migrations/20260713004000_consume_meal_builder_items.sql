...
declare
  ...
  v_locked_stock_quantities numeric[] := '{}';
  v_index integer;
...
begin
  ...
  for v_index in 1..v_line_count loop
    ...
    v_locked_stock_quantities := array_append(v_locked_stock_quantities, v_item.quantity);
  ...
  for v_index in 1..v_line_count loop
    if v_sorted_quantities[v_index] < v_locked_stock_quantities[v_index] then
      update public.inventory_items
      set quantity = v_locked_stock_quantities[v_index] - v_sorted_quantities[v_index]
      where id = v_sorted_item_ids[v_index]
        and user_id = v_user_id
        and quantity = v_locked_stock_quantities[v_index];
    else
