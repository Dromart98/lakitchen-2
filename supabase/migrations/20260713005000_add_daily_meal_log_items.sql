create table public.daily_meal_log_items (
  ...
  constraint daily_meal_log_items_product_name_not_empty check (char_length(btrim(product_name)) > 0),
  constraint daily_meal_log_items_product_name_length check (char_length(product_name) <= 120),
  constraint daily_meal_log_items_consumed_quantity_positive check (consumed_quantity not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric) and consumed_quantity > 0),
  constraint daily_meal_log_items_unit_check check (unit in ('g', 'kg', 'ml', 'l', 'ud')),
  constraint daily_meal_log_items_nutrition_basis_check check (nutrition_basis in ('per_100g', 'per_100ml', 'per_unit')),
  constraint daily_meal_log_items_calories_non_negative check (calories not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric) and calories >= 0),
  constraint daily_meal_log_items_protein_g_non_negative check (protein_g not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric) and protein_g >= 0),
  constraint daily_meal_log_items_carbs_g_non_negative check (carbs_g not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric) and carbs_g >= 0),
  constraint daily_meal_log_items_fat_g_non_negative check (fat_g not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric) and fat_g >= 0),
  constraint daily_meal_log_items_meal_source_unique unique (meal_log_id, source_inventory_item_id)
);

alter table public.daily_meal_log_items enable row level security;

revoke all on table public.daily_meal_log_items from anon;
revoke all on table public.daily_meal_log_items from public;
revoke all on table public.daily_meal_log_items from authenticated;
grant select, insert on table public.daily_meal_log_items to authenticated;

create index daily_meal_log_items_meal_log_id_idx
on public.daily_meal_log_items (meal_log_id);

create index daily_meal_log_items_user_id_idx
on public.daily_meal_log_items (user_id);

create index daily_meal_log_items_created_at_idx
on public.daily_meal_log_items (created_at);

create policy "Users can view own daily meal log items"
on public.daily_meal_log_items
for select
to authenticated
using ((select auth.uid()) = user_id);

create policy "Users can create own daily meal log items"
on public.daily_meal_log_items
for insert
to authenticated
...
begin
  ...
  if p_meal_name is null
    ...
    or char_length(p_meal_name) > 120 then
  ...
  loop
    ...
    if not found or v_item.user_id <> v_user_id then
  ...
  for v_line in
    ...
    order by item_id
  loop
    select quantity - v_line.consumed_quantity
