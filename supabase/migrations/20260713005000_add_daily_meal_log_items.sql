create table public.daily_meal_log_items (
  ...
  constraint daily_meal_log_items_product_name_length check (char_length(btrim(product_name)) <= 120),
  constraint daily_meal_log_items_consumed_quantity_positive check (
    consumed_quantity not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
    and consumed_quantity > 0
  ),
  constraint daily_meal_log_items_unit_check check (unit in ('g', 'kg', 'ml', 'l', 'ud')),
  constraint daily_meal_log_items_nutrition_basis_check check (
    nutrition_basis in ('per_100g', 'per_100ml', 'per_unit')
  ),
  constraint daily_meal_log_items_calories_non_negative check (
    calories not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
    and calories >= 0
  ),
  constraint daily_meal_log_items_protein_g_non_negative check (
    protein_g not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
    and protein_g >= 0
  ),
  constraint daily_meal_log_items_carbs_g_non_negative check (
    carbs_g not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
    and carbs_g >= 0
  ),
  constraint daily_meal_log_items_fat_g_non_negative check (
    fat_g not in ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)
    and fat_g >= 0
  ),
  constraint daily_meal_log_items_meal_source_unique unique (
    meal_log_id,
...
with check (
  ...
  and exists (
    select 1
    from public.daily_meal_logs as meal
    where meal.id = daily_meal_log_items.meal_log_id
