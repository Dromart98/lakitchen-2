...
revoke all on table public.inventory_batch_submissions from public, anon;
grant select, insert on table public.inventory_batch_submissions to authenticated;
...
begin
  ...
  for v_item in select value from pg_catalog.jsonb_array_elements(p_items) loop
    if pg_catalog.jsonb_typeof(v_item) <> 'object' or (select count(*) from pg_catalog.jsonb_object_keys(v_item) k where k not in ('name','quantity','unit','location','category','nutrition_basis','calories','protein_g','carbs_g','fat_g')) <> 0 or (select count(*) from pg_catalog.jsonb_object_keys(v_item)) <> 10 then raise exception using errcode = '22023', message = 'invalid-batch-payload'; end if;
