...
begin
  ...
  loop
    ...
    if v_item.nutrition_basis = 'per_100g' and v_item.unit = 'g' then
      ...
      v_equivalence_count := 0;
      -- Lock every candidate, before inspecting its value, so ambiguity fails closed.
      ...
      if v_equivalence_count <> 1
        or v_equivalence.canonical_quantity is null
      ...
      if v_item.nutrition_basis = 'per_100g' and v_item.unit = 'ud' and v_equivalence.canonical_unit = 'g' then
        v_factor := v_line.consumed_quantity * v_equivalence.canonical_quantity / 100;
      elsif v_item.nutrition_basis = 'per_100ml' and v_item.unit = 'ud' and v_equivalence.canonical_unit = 'ml' then
        v_factor := v_line.consumed_quantity * v_equivalence.canonical_quantity / 100;
      elsif v_item.nutrition_basis = 'per_unit' and v_item.unit = 'g' and v_equivalence.canonical_unit = 'g' then
        v_factor := v_line.consumed_quantity / v_equivalence.canonical_quantity;
      elsif v_item.nutrition_basis = 'per_unit' and v_item.unit = 'kg' and v_equivalence.canonical_unit = 'g' then
        v_factor := v_line.consumed_quantity * 1000 / v_equivalence.canonical_quantity;
      elsif v_item.nutrition_basis = 'per_unit' and v_item.unit = 'ml' and v_equivalence.canonical_unit = 'ml' then
        v_factor := v_line.consumed_quantity / v_equivalence.canonical_quantity;
      elsif v_item.nutrition_basis = 'per_unit' and v_item.unit = 'l' and v_equivalence.canonical_unit = 'ml' then
        v_factor := v_line.consumed_quantity * 1000 / v_equivalence.canonical_quantity;
      else
        raise exception using errcode = '22023', message = 'Incompatible inventory nutrition unit';
      end if;
