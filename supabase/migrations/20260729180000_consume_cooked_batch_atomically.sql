-- A batch and its meal remain an indivisible accounting record. Composite
-- ownership keys prevent a definer function from ever linking across users.
alter table public.user_saved_ai_recipe_cooked_batches
  ...
  add constraint user_saved_ai_recipe_cooked_batches_id_user_key unique (id, user_id);
...
create table public.user_saved_ai_recipe_cooked_batch_consumptions (
  ...
  requested_mode text not null check (requested_mode in ('servings', 'cooked_weight_g')),
  ...
  constraint cooked_batch_consumptions_requested_quantity_check
    check (requested_quantity > 0 and requested_quantity not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)),
  constraint cooked_batch_consumptions_consumed_weight_check
    check (consumed_cooked_weight_g > 0 and consumed_cooked_weight_g not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)),
  constraint cooked_batch_consumptions_consumed_servings_check
    check (consumed_servings > 0 and consumed_servings not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)),
  constraint cooked_batch_consumptions_nutrition_check
    check (
      consumed_calories >= 0 and consumed_calories not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)
      and consumed_protein_g >= 0 and consumed_protein_g not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)
      and consumed_carbs_g >= 0 and consumed_carbs_g not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)
      and consumed_fat_g >= 0 and consumed_fat_g not in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision)
    ),
  constraint cooked_batch_consumptions_fingerprint_check
    check (idempotency_fingerprint = btrim(idempotency_fingerprint) and idempotency_fingerprint <> '')
);
...
alter table public.user_saved_ai_recipe_cooked_batch_consumptions enable row level security;
alter table public.user_saved_ai_recipe_cooked_batch_consumptions force row level security;
...
revoke all on table public.user_saved_ai_recipe_cooked_batch_consumptions from anon;
revoke all on table public.user_saved_ai_recipe_cooked_batch_consumptions from authenticated;
grant select on table public.user_saved_ai_recipe_cooked_batch_consumptions to authenticated;
...
begin
  ...
  if p_request_id is null or p_batch_id is null or p_expected_batch_updated_at is null
  ...
  v_mode := case when p_servings_consumed is not null then 'servings' else 'cooked_weight_g' end;
  v_requested := coalesce(p_servings_consumed, p_cooked_weight_consumed_g);
  if v_requested <= 0 or v_requested in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision) then
    raise exception using errcode = '22023', message = 'invalid_input';
  end if;
  v_fingerprint := concat_ws('|', p_batch_id::text, p_meal_type,
    p_expected_batch_updated_at::text, v_mode, v_requested::text);

  -- Serialize the request key so concurrent retries also observe the first
  -- committed event before any mutable batch validation.
  ...
  for update;
  if found then
    if v_existing.user_id <> v_user_id or v_existing.idempotency_fingerprint <> v_fingerprint then
      raise exception using errcode = '23505', message = 'idempotency_conflict';
    end if;
    return query select v_existing.consumed_cooked_weight_g, v_existing.consumed_servings,
      v_existing.consumed_calories, v_existing.consumed_protein_g,
      v_existing.consumed_carbs_g, v_existing.consumed_fat_g;
    return;
  end if;
  ...
  v_tolerance := 2.220446049250313e-16::double precision
    * greatest(1::double precision, abs(v_batch.cooked_weight_g), abs(v_remaining)) * 8;
  ...
  if v_weight <= 0 or v_weight in ('Infinity'::double precision, '-Infinity'::double precision, 'NaN'::double precision) then
    raise exception using errcode = '22023', message = 'invalid_input';
  end if;
  ...
  insert into public.daily_meal_logs (
    user_id, name, calories, protein_g, carbs_g, fat_g, consumed_on, meal_type
  ) values (
    v_user_id, v_batch.recipe_title, calories, protein_g, carbs_g, fat_g,
    (pg_catalog.now() at time zone 'utc')::date, p_meal_type
  ) returning id into v_meal_id;

  insert into public.user_saved_ai_recipe_cooked_batch_consumptions (
    request_id, user_id, batch_id, meal_log_id, requested_mode, requested_quantity,
    consumed_cooked_weight_g, consumed_servings, consumed_calories,
    consumed_protein_g, consumed_carbs_g, consumed_fat_g, idempotency_fingerprint
  ) values (
    p_request_id, v_user_id, p_batch_id, v_meal_id, v_mode, v_requested,
    v_weight, v_servings, calories, protein_g, carbs_g, fat_g, v_fingerprint
  );
...
revoke execute on function public.set_cooked_batch_monotonic_version() from public;
revoke execute on function public.set_cooked_batch_monotonic_version() from anon;
revoke execute on function public.set_cooked_batch_monotonic_version() from authenticated;
revoke execute on function public.consume_cooked_batch_and_log_meal(uuid, uuid, text, timestamptz, double precision, double precision) from public;
revoke execute on function public.consume_cooked_batch_and_log_meal(uuid, uuid, text, timestamptz, double precision, double precision) from anon;
grant execute on function public.consume_cooked_batch_and_log_meal(uuid, uuid, text, timestamptz, double precision, double precision) to authenticated;
