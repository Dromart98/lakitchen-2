...
**Estado: completada. Prioridad: alta.** Runbook operativo documentado en `docs/rollback.md`. El simulacro real no destructivo promovió una release anterior compatible y restauró después el deployment original en 2 s por cambio, con liveness/readiness `200` y `no-store`. La validación posterior detectó y permitió corregir drift de migraciones aditivas y un bloqueo opcional de catálogo en Inventario; tras la corrección, la producción en `22f930aaf35d7d2feab559b5b2b8be4837844872` quedó `READY` y `Authenticated E2E` completó todas las suites críticas en verde. El registro completo del simulacro queda en `docs/rollback.md`.
