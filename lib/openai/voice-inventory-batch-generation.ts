...
export const VOICE_INVENTORY_BATCH_JSON_SCHEMA = { type: "object", properties: { items: { type: "array", minItems: 1, maxItems: VOICE_INVENTORY_BATCH_MAX_ITEMS, items: { type: "object", properties: { name: { type: "string" }, quantity: { anyOf: [{ type: "number" }, { type: "null" }] }, unit: { anyOf: [{ type: "string", enum: ["g", "kg", "ml", "l", "ud"] }, { type: "null" }] }, location: { anyOf: [{ type: "string", enum: ["pantry", "fridge", "freezer"] }, { type: "null" }] }, category: { anyOf: [{ type: "string", enum: ["protein", "carbohydrate", "vegetable", "fruit", "fat", "dairy", "legume", "condiment", "beverage", "other"] }, { type: "null" }] }, food_state: { type: "string", enum: ["raw", "cooked", "processed", "not_applicable", "unknown"] }, nutrition_basis: { anyOf: [{ type: "string", enum: ["per_100g", "per_100ml", "per_unit"] }, { type: "null" }] }, calories: { anyOf: [{ type: "number" }, { type: "null" }] }, protein_g: { anyOf: [{ type: "number" }, { type: "null" }] }, carbs_g: { anyOf: [{ type: "number" }, { type: "null" }] }, fat_g: { anyOf: [{ type: "number" }, { type: "null" }] }, confidence: { type: "string", enum: ["high", "medium", "low"] }, nutrition_assumptions: { type: "string" }, package_count: { anyOf: [{ type: "number" }, { type: "null" }] }, package_measure_kind: { anyOf: [{ type: "string", enum: ["can", "package"] }, { type: "null" }] }, package_size: { anyOf: [{ type: "number" }, { type: "null" }] }, package_size_unit: { anyOf: [{ type: "string", enum: ["g", "kg", "ml", "l"] }, { type: "null" }] }, total_size: { anyOf: [{ type: "number" }, { type: "null" }] }, total_size_unit: { anyOf: [{ type: "string", enum: ["g", "kg", "ml", "l"] }, { type: "null" }] }, issues: { type: "array", items: { type: "string", enum: ["quantity-missing", "unit-missing", "location-unconfirmed", "package-size-missing", "package-kind-missing", "nutrition-incomplete", "low-confidence", "ambiguous-product"] } } }, required: ["name", "quantity", "unit", "location", "category", "food_state", "nutrition_basis", "calories", "protein_g", "carbs_g", "fat_g", "confidence", "nutrition_assumptions", "package_count", "package_measure_kind", "package_size", "package_size_unit", "total_size", "total_size_unit", "issues"], additionalProperties: false } } }, required: ["items"], additionalProperties: false } as const;
const prompt = `${INVENTORY_NUTRITION_AI_SYSTEM_PROMPT}\n\n${buildInventoryDefaultRawFoodPromptInstruction()}\n\nExtrae una lista de inventario en español y estima la nutrición de cada alimento identificable aplicando exactamente las reglas anteriores. Devuelve hasta 30 elementos en el mismo orden. gramos/kg usan per_100g, ml/l per_100ml y ud solo para unidades reales. Las macros son por la base, nunca por el total de inventario. despensa=pantry, nevera/refrigerador/frigorífico=fridge, congelador=freezer. Cuando el usuario introduzca una sección como «en la nevera tengo…», «congelador: …» o «en la despensa hay…», aplica esa ubicación a todos los productos siguientes hasta que aparezca otra sección; una ubicación explícita de un producto prevalece sobre la heredada. Si no se indica ubicación, añade location-unconfirmed. Limítate a extraer hechos: para N envases usa package_count=N y quantity=N, unit=ud. Solo si el usuario menciona explícitamente el envase, usa package_measure_kind=can para lata y package_measure_kind=package para paquete, bolsa, caja, botella o bote; si hay package_count sin tipo añade package-kind-missing. No infieras el tipo de envase por el alimento. No uses metadatos de envase para alimentos naturalmente contables como manzanas o huevos. Si se expresa tamaño individual, rellena package_size y package_size_unit; si se expresa un total, rellena total_size y total_size_unit. No multipliques, dividas ni conviertas: deja esa aritmética al código. Para esos productos devuelve la nutrición base por 100 g o 100 ml, nunca calculada por unidad. Usa null en todo metadato no expresado. Para envases sin peso o volumen explícito, no inventes su tamaño ni su nutrición: añade package-size-missing y nutrition-incomplete. Para cantidades simples como 500 g de arroz, conserva quantity=500 y unit=g y deja todos los metadatos de envase, incluido package_measure_kind, en null. Para un alimento ambiguo o que necesita aclaración deja todos los valores nutricionales null, explica el motivo en nutrition_assumptions y añade ambiguous-product y nutrition-incomplete. Para una estimación, nutrition_assumptions debe indicar brevemente que son valores típicos por la base. No sustituyas valores nutricionales explícitamente dictados.`;
