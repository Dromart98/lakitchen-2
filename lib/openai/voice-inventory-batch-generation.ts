...
export const VOICE_INVENTORY_BATCH_JSON_SCHEMA = { type: "object", properties: { items: { type: "array", minItems: 1, maxItems: VOICE_INVENTORY_BATCH_MAX_ITEMS, items: { type: "object", properties: { name: { type: "string" }, quantity: { anyOf: [{ type: "number" }, { type: "null" }] }, unit: { anyOf: [{ type: "string", enum: ["g", "kg", "ml", "l", "ud"] }, { type: "null" }] }, location: { anyOf: [{ type: "string", enum: ["pantry", "fridge", "freezer"] }, { type: "null" }] }, category: { anyOf: [{ type: "string", enum: ["protein", "carbohydrate", "vegetable", "fruit", "fat", "dairy", "legume", "condiment", "beverage", "other"] }, { type: "null" }] }, food_state: { type: "string", enum: ["raw", "cooked", "processed", "not_applicable", "unknown"] }, nutrition_basis: { anyOf: [{ type: "string", enum: ["per_100g", "per_100ml", "per_unit"] }, { type: "null" }] }, calories: { anyOf: [{ type: "number" }, { type: "null" }] }, protein_g: { anyOf: [{ type: "number" }, { type: "null" }] }, carbs_g: { anyOf: [{ type: "number" }, { type: "null" }] }, fat_g: { anyOf: [{ type: "number" }, { type: "null" }] }, confidence: { type: "string", enum: ["high", "medium", "low"] }, nutrition_assumptions: { type: "string" }, issues: { type: "array", items: { type: "string", enum: ["quantity-missing", "unit-missing", "location-unconfirmed", "package-size-missing", "nutrition-incomplete", "low-confidence", "ambiguous-product"] } } }, required: ["name", "quantity", "unit", "location", "category", "food_state", "nutrition_basis", "calories", "protein_g", "carbs_g", "fat_g", "confidence", "nutrition_assumptions", "issues"], additionalProperties: false } } }, required: ["items"], additionalProperties: false } as const;
const prompt = `${INVENTORY_NUTRITION_AI_SYSTEM_PROMPT}\n\nExtrae una lista de inventario en español y estima la nutrición de cada alimento identificable aplicando exactamente las reglas anteriores. Devuelve hasta 30 elementos en el mismo orden. gramos/kg usan per_100g, ml/l per_100ml y ud solo para unidades reales. Las macros son por la base, nunca por el total de inventario. despensa=pantry, nevera/refrigerador/frigorífico=fridge, congelador=freezer. Si no se indica ubicación, añade location-unconfirmed. Dos paquetes, bolsa, caja, bote o manojo sin peso: conserva el nombre, deja cantidad y unidad null, no estimes nutrición y añade package-size-missing y nutrition-incomplete. Para un alimento ambiguo o que necesita aclaración deja todos los valores nutricionales null, explica el motivo en nutrition_assumptions y añade ambiguous-product y nutrition-incomplete. Para una estimación, nutrition_assumptions debe indicar brevemente que son valores típicos por la base. No sustituyas valores nutricionales explícitamente dictados.`;
...
export async function generateVoiceInventoryBatch(text: string, options: { apiKey: string; model?: string; fetchImpl?: typeof fetch }): Promise<VoiceInventoryBatchResult> {
 ...
 const nutritionIsValid = validated.data.items.every((item) => {
   const nutritionValues = [item.nutrition_basis, item.calories, item.protein_g, item.carbs_g, item.fat_g];
   if (nutritionValues.every((value) => value === null)) return true;
   if (item.unit === null || item.nutrition_basis === null) return false;
   const result = validateInventoryNutritionAiOutput({ name: item.name, quantity: item.quantity, unit: item.unit, category: item.category }, { status: item.calories === null || item.protein_g === null || item.carbs_g === null || item.fat_g === null ? "needs_clarification" : "estimated", nutrition_basis: item.nutrition_basis, calories: item.calories, protein_g: item.protein_g, carbs_g: item.carbs_g, fat_g: item.fat_g, confidence: item.confidence, food_state: item.food_state, normalized_food_name: item.name, assumptions: item.nutrition_assumptions, clarification: item.calories === null ? item.nutrition_assumptions : null });
   return result.status !== "invalid";
 });
 if (!nutritionIsValid) return { status: "error", code: "invalid-ai-response", message: "No se pudo obtener una estimación nutricional válida." }; const items = withDraftClientIds(validated.data.items); return items.some((item) => getVoiceInventoryDraftStatus(item) !== "Listo") ? { status: "needs-clarification", items, message: "Revisa los productos marcados antes de la próxima fase." } : { status: "success", items };
