...
function extract(body: unknown): string | null { if (!record(body) || body.status !== "completed") return null; if (typeof body.output_text === "string" && body.output_text.trim()) return body.output_text; if (!Array.isArray(body.output)) return null; for (const item of body.output) if (record(item) && Array.isArray(item.content)) for (const content of item.content) if (record(content) && content.type === "output_text" && typeof content.text === "string") return content.text; return null; }
...
export async function estimateTextMealWithOpenAi(description: string, options: { apiKey: string; model?: string; fetchImpl?: typeof fetch }): Promise<TextMealEstimationResult> { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), TEXT_MEAL_AI_TIMEOUT_MS); try { const response = await (options.fetchImpl ?? fetch)(OPENAI_RESPONSES_ENDPOINT, { method: "POST", headers: { Authorization: `Bearer ${options.apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: options.model ?? TEXT_MEAL_AI_MODEL_DEFAULT, input: [{ role: "system", content: TEXT_MEAL_SYSTEM_PROMPT }, { role: "user", content: description }], text: { format: { type: "json_schema", name: "text_meal_estimation", strict: true, schema: TEXT_MEAL_JSON_SCHEMA } }, store: false, reasoning: { effort: "low" }, max_output_tokens: 2500 }), signal: controller.signal }); if (response.status === 408) return { status: "error", code: "provider-timeout" }; if (!response.ok) return { status: "error", code: "provider-error" }; const text = extract(await response.json()); if (!text) return { status: "error", code: "invalid-ai-response" }; try { return validateTextMealProviderOutput(normalizeTextMealProviderOutput(JSON.parse(text))); } catch { return { status: "error", code: "invalid-ai-response" }; } } catch (error) { return record(error) && error.name === "AbortError" ? { status: "error", code: "provider-timeout" } : { status: "error", code: "provider-error" }; } finally { clearTimeout(timeout); } }
