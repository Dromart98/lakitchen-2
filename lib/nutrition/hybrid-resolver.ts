...
export async function resolveInventoryNutrition(input: InventoryNutritionAiInput, options: { usdaApiKey?: string; openAiApiKey?: string; fetchImpl?: typeof fetch } = {}): Promise<NutritionResolution> {
  ...
  const ai = await estimateInventoryNutritionWithOpenAi(input, { apiKey, fetchImpl: options.fetchImpl });
