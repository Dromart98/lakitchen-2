...
describe("inventory barcode camera lifecycle", () => {
  ...
  it("prefers a useful rear-camera stream and progressively configures supported autofocus", () => {
    ...
    expect(controls).toContain("selectFocusMode(capabilities)");
    expect(controls).toContain("await track.applyConstraints(constraints)");
    expect(controls).toContain("track.getSettings?.()");
  });
