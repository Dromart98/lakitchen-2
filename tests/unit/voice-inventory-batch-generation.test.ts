...
describe("voice inventory batch provider", () => {
  it("posts a strict, private structured request and accepts root output", async () => {
    ...
    expect(body.input[0].content).toContain("clasifícalos como raw");
  });
  ...
  it("requires the provider to estimate unprepared rice as raw and rejects cooked rice values", async () => {
    ...
    expect(success).toMatchObject({ status: "success", items: [expect.objectContaining({ quantity: 1, unit: "kg", location: "pantry", category: "carbohydrate", food_state: "raw", nutrition_basis: "per_100g", calories: 360 })] });
    expect(rejected).toMatchObject({ status: "error", code: "invalid-ai-response" });
  });
  it("preserves explicit cooked rice rather than applying the raw default", async () => {
