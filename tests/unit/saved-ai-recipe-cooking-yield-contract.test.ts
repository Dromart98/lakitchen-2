...
describe("saved AI recipe cooking yield RSC contract", () => {
  it("projects only public totals or an incomplete review count to the client", () => {
    ...
    expect(preview).toContain('nutrition }: { recipeId: string; nutrition: SavedRecipeCookingYieldNutrition }');
  ...
  it("keeps state inside each keyed recipe preview and starts closed", () => {
    expect(preview).toContain("useState<Fields>(INITIAL_FIELDS)");
    expect(preview).toContain("useState<CookingYieldResult | null>(null)");
