...
function getValidatedInventoryFields(formData: FormData) {
  ...
  if (!name) {
    redirect(`${INVENTORY_PATH}?inventoryError=name-required`);
  }

  if (name.length > 120) {
    redirect(`${INVENTORY_PATH}?inventoryError=name-too-long`);
  }
  ...
  if (!isInventoryUnit(unit)) {
    redirect(`${INVENTORY_PATH}?inventoryError=invalid-unit`);
  }

  if (!isInventoryLocation(location)) {
    redirect(`${INVENTORY_PATH}?inventoryError=invalid-location`);
  }
...
export async function addInventoryItemAction(formData: FormData) {
  const { name, quantity, unit, location } = getValidatedInventoryFields(formData);
...
export async function deleteInventoryItemAction(formData: FormData) {
  ...
  if (!isUuid(id)) {
    redirect(`${INVENTORY_PATH}?inventoryError=delete-not-found`);
  }
