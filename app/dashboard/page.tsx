...
export default async function DashboardPage() {
  ...
  return <main className="shell"><div className="topbar"><h1>Lakitchen</h1><form action="/auth/signout" method="post"><button className="logout-link" type="submit">Cerrar sesión</button></form></div><p className="muted">Dashboard mobile-first para macros, inventario y recetas.</p><div className="dashboard-actions"><Link className="button nav-button" href="/nutrition-profile">Configurar perfil nutricional</Link></div><section className="grid cards"><MacroProgress consumed={consumedToday} goal={todayGoal}/><div className="card"><h2>Restante</h2><p>{remaining.calories} kcal</p><p className="muted">P {remaining.proteinG}g · C {remaining.carbsG}g · G {remaining.fatG}g</p></div></section><section className="grid cards" style={{marginTop:16}}><ExpiringList items={expiring}/><RecipeSuggestion recipe={recipe}/></section></main>;
}
