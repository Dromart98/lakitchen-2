...
function getMealErrorMessage(code: string | undefined) {
  ...
  if (code === "meal-not-found") return "No se encontró la comida registrada.";
        ...
        <div className="card">
          ...
          <form action={addMealLogAction} className="meal-log-form">
            <label className="field" htmlFor="meal-name">
              <span>Nombre</span>
              <input id="meal-name" name="name" type="text" maxLength={120} required placeholder="Pollo con arroz" />
            </label>
            ...
            <ul>
              {mealsToday.map((meal) => (
                <li key={meal.id}>
                  ...
                  <form action={deleteMealLogAction} className="meal-log-form">
